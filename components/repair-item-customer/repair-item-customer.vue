<template>
	<view>
		<uni-list-item v-if="Boolean(item)">
			<template slot="body" style="width: 100%; padding-0">
				<navigator :url="'/pages_repair-order/customer/detail?preview=true&OrderId=' + item.OrderId ">
					<!-- 工单号和状态 -->
					...
					<view class="flex flex-direction padding-lr-sm text-df">
						...
						<view v-if="Boolean(item.OrderType)" class="box">
							...
							<view class="item-right">{{ item.OrderType === "SnOrder"?i18n_order.repair.snOrder:i18n_order.repair.cateOrder }}
							 </view>
						</view>
						<!-- SN报障 -->
						...
						<!-- 服务商 -->
						<view v-if="Boolean(item.SupplierMchName)" class="box">
							<view class="item-left">{{ i18n_order.repair.svcName }}</view>
							<view class="item-right ">{{ item.SupplierMchName }}</view>
						</view>
						<!-- 报障时间 -->
						<view class="box">
